...
{{-- <div class="row">
    @if ($mergedResults->isNotEmpty())
            ...
            <div class="col-md-4 mt-3">
                <div class="card job-card">
                    <div class="card-body">
                        <div class="row">
                            <div>
                                <h5 class="card-title"><strong>{{ $item->title_}}</strong></h5>
                                <p class="card-text">{{ $item->company_name}}</p>
                                <p class="card-text">Location: {{ $item->district}}</p>
                                <p class="card-text">Skills: {{ $item->required_skills}}</p>
                                <p class="card-text"><small class="text-muted">
                            ...
                            <div>
                                @if ($item->type == "Intern")
                                <a href="{{ url('internship_detail/'.$item->id) }}"><button class="btn btn-primary view-more-btn">View More</button></a>
                                @else
                                <a href="{{ url('job_detail/'.$item->id) }}"><button class="btn btn-primary view-more-btn">View More</button></a>
                                @endif
        ...
        @endforeach
         ...
         {{ $mergedResults->links('guest.link') }}

...
let loading = document.getElementById('loading');
            ...
            fetch("{{ route('search_rejected_internships') }}?title=" + queryString)
                .then(response => response.text())
                .then(data => {
...
let loading = document.getElementById('loading');
    ...
    fetch("{{ route('search_rejected_jobs') }}?title=" + queryString)
        .then(response => response.text())
        .then(data => {
            loading.classList.add('d-none');
            document.getElementById('jobs-container').innerHTML = "";
    ...
    function viewInt(){
        let loading = document.getElementById('loading');
            loading.classList.remove('d-none');
        fetch('/rejected_application_intern')
                .then(response => {
                    if (!response.ok) {
...
loading.classList.add('d-none');
    ...
    function viewJob(){
        let loading = document.getElementById('loading');
            loading.classList.remove('d-none');
        fetch('/rejected_application_job')
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Network response was not ok');
                    }

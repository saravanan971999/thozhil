            ...
            @endforeach
                        ...
                        <li class="nav-item dropdown no-arrow">
                            <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button"
                                ...
                                <span class="mr-2 d-none d-lg-inline text-gray-600 small"> {{$userr->full_name}}</span>
                            </a>
                ...
                <div class="container-fluid">
                    ...
                    <center> <h1 style="color: black;">Test Schedule</h1></center>
                                            ...
                                            <div class="booking_card">
                                                <div class="table-responsive">
                                                    ...
                                                    <div class="container mt-5">
                                                        ...
                                                        <table class="text-center table table-bordered custom-table">
                                                          <thead>
                                                            <tr>
                                                              <th>S.No</th>
                                                    ...
                                                    @if ($applications->count() > 0)
                                                        @foreach ($applications as $us)
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <a href="{{url('candidate/application_profile/'.$us->application_id)}}" style="font-size:20px; font-style:bold;">Track Development</a>
                                                                </td>

                                                                <td>
                                                                    @if ($us->test_status == 1)
                                                                        @if ($us->zoom_meeting != null)
                                                                            @if(\Carbon\Carbon::parse($us->zoom_time)->isFuture())
                                                                                {{ \Carbon\Carbon::parse($us->zoom_time)->diffForHumans(null, true) }} to go

                                                                            @endif
                                                                        @endif
                                                                    @elseif ($us->test_status == 0)

                                                                        @php
                                                                            $conductingDateTime = \Carbon\Carbon::parse($us->conducting_datetime);
                                                                            $now = now();
                                                                            $difference = $now->diffInSeconds($conductingDateTime, false);
                                                                        @endphp

                                                                        @if($conductingDateTime->isFuture())
                                                                            {{ $conductingDateTime->diffForHumans(null, true) }} to go ,
                                                                        @elseif($difference <= 3600 && $difference  >= -3600)
                                                                            {{ $now->diffForHumans($conductingDateTime, true) }} ago



                                                                        @else
                                                                            @if ($us->test_qualified == 1)
                                                                                @if ($us->zoom_meeting != null)
                                                                                    @if(\Carbon\Carbon::parse($us->zoom_time)->isFuture())
                                                                                        {{ \Carbon\Carbon::parse($us->zoom_time)->diffForHumans(null, true) }} to go
                                                                                    @endif
                                                                                @endif
                                                                            @elseif (is_null($us->testsss_status))
                                                                                You missed the test

                                                                            @endif
                                                                        @endif
                                                                    @elseif ($us->status == 1 && is_null($us->quiz_id))
                                                                        @if ($us->zoom_meeting != null)
                                                                            @if(\Carbon\Carbon::parse($us->zoom_time)->isFuture())
                                                                                {{ \Carbon\Carbon::parse($us->zoom_time)->diffForHumans(null, true) }} to go
                                                                            @endif

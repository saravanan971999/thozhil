...
<div class="page-wrapper">
    <div class="content container-fluid">
        <div class="background-image">
            <img src="{{ asset('asset/emplanding/test2.svg')}}" alt="Background Image">
        </div>
        ...
        <div class="container">
            ...
            @if (isset($jobid))
                @php
                    $encryptedJobId = encrypt($jobid);
                    $encryptedJri = encrypt($jri);
                @endphp
        <form action="{{ url('adding_questions/'.$encryptedJobId.'/'.$encryptedJri) }}" method="POST" id="quizForm">

            @else
                <form action="{{ url('adding_questions_empty/') }}" method="POST" id="quizForm">
            @endif

                @csrf
                {{-- <input type="hidden" name="jobID" value="{{}}" readonly>
                <input type="hidden" name="jri" value="{{$}}" readonly> --}}
